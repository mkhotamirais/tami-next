...
let x: { id: number } & { name: string } = { id: 10, name: "ahamd" };
export default function Ts1() {
  return (
    <div className="min-h-screen px-3 md:px-16 text-sm">
      <p>
        syaratnya harus terinstall <a href="https://nodejs.org/en">nodejs</a> nodejs dan typescript (npm i -g
        typescript) lalu cek (node -v; npm -v; npx -v; tsx -v). buat folder bebas di mana saja lalu buat file, contoh
        index.ts. isi apa saja lalu jalankan terminal (tsc index.ts) maka akan terbentuk file index.js hasil kompilasi
        index.ts. selanjutnya coba buat yang baru dengan tsconfig.json. caranya buat folder, buka di terminal, jalankan
        (tsc --init) akan terbentuk tsconfig.json yang berisi banyak kofigurasi. <strong>penting:</strong> Setiap nilai
        memiliki tipe data tertentu, jika tipe suatu variabel telah ditentukan maka nilainya tidak bisa diubah dengan
